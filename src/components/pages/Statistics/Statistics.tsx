...
const Statistics = (
  ...
  const [docStat, setDocStat] = useState<DoctorsStats | undefined>();
  ...
  const { isLoading } = useQuery(["stats", doctorId], fetchStats, {
    onSuccess: (data) => {
      ...
      if (source === 'allDoctors') {
        if (data && data.doctorsStats) {
          setAllDocStats(data.doctorsStats);
        }
      }
    },
  });

  if (docStat && source === 'doctor') {
  ...
  if (source === 'allDoctors') {
    const allStats = allDocStats.map((doc) => {
      const id: string = doc.doctorName;
      const label: string = doc.doctorName;
      const value: number =
        doc.doctorAppointmentsStats.numberOfRescheduled +
        doc.doctorAppointmentsStats.numberOfDone +
        doc.doctorAppointmentsStats.numberOfNew +
        doc.doctorAppointmentsStats.numberOfApproved +
        doc.doctorAppointmentsStats.numberOfCanceled;
